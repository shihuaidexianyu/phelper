...
**M0 (read-only probe), M1 (telemetry engine), M2 (control foundation), and M3 (power-limits special: PERFEPP1 + 0x22 + 0x29) are complete and verified on the reference machine.** Workspace: `phelper-domain` (pure domain model + ports) → `phelper-core` (engine + platform adapters) → `phelper-cli` (dev/verification harness). UI is deferred; core-first (telemetry + control + policy).
...
- `cargo build --workspace` / `cargo test --workspace` / `cargo clippy --workspace` (all clean; 69 tests default / 74 with `--all-features`)
